<?php
    ...
    if(!isset($_SESSION['logged_id'])){
        ...
        $sql_show_expenses = $db->query("SELECT e.date, e.money, ec.name, ep.name, e.comment 
            FROM expenses AS e INNER JOIN expense_categories AS ec ON e.user_id = ec.user_id AND 
            e.expense_type_id=ec.id INNER JOIN payment_methods AS ep ON e.user_id = ep.user_id AND 
            e.pay_method_id = ep.id WHERE e.user_id=$user_id AND date BETWEEN DATE_SUB(CURDATE(),
                INTERVAL (DAY(CURDATE())-1) DAY) AND LAST_DAY(NOW())");
        $expenses=$sql_show_expenses->fetchAll();

        $sql_sum_expenses = $db->query("SELECT ROUND(SUM(money),2) as totalAmount FROM expenses 
            WHERE user_id=$user_id AND date BETWEEN DATE_SUB(CURDATE(), INTERVAL (DAY(CURDATE())-1) DAY) AND LAST_DAY(NOW())");
        ...
        <div class="container-fluid mainContent justify-content-center">
            <div class="info-board d-flex justify-content-between text-center mb-3 ml-3">
                <div class="totalIncomesBox">
                    <h3>SUMA WYDATKÓW</h3>
                    <p class="incomeBox">w bieżącym miesiącu</p>
