<?php
    ...
    if(!isset($_SESSION['logged_id'])){
        ...
        $today = date('Y-m-d');
        $sql_show_incomes = $db->query("SELECT e.date, e.money, ec.name, ep.name, e.comment 
            ...
            e.pay_method_id = ep.id WHERE e.user_id=31 AND date BETWEEN DATE_ADD(CURDATE(), 
            INTERVAL 1-DAYOFWEEK(CURDATE()) DAY) AND DATE_ADD(CURDATE(), INTERVAL 7-DAYOFWEEK(CURDATE()) DAY)");
